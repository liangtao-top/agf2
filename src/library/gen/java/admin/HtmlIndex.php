...
use com\agf2\util\ImportJava;

class HtmlIndex extends LibJavaAdminAbs
{
        ...
        self::each($data, function ($value, $field) use (&$code) {
            switch ((int)$field['type_id']) {
                ...
                case 7:
                    ...
                    $code[]     = $selectType === 1 ? ImportJava::css('/assets/vendor/select2/select2.min.css') : ImportJava::css('/assets/vendor/bootstrap-select/bootstrap-select.min.css');
                    break;
            }
        });
    ...
    private static function vendorJs(array &$data): string
    {
        ...
        self::each($data, function ($value, $field) use (&$code) {
            switch ((int)$field['type_id']) {
                ...
                case 7:
                    $selectType = isset($field['formData']['selectType']) ? (int)$field['formData']['selectType'] : 0;
                    $code[]     = $selectType === 1 ? ImportJava::js(['/assets/vendor/select2/select2.full.min.js', '/assets/vendor/select2/i18n/zh-CN.js']) : ImportJava::js('/assets/vendor/bootstrap-select/bootstrap-select.min.js');
                    break;
            }
        });
        return Helper::format($code);
...
html;
    ...
    {
        $html = [];
        self::each($data, function ($value, $field) use (&$html) {
